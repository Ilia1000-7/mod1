...
LENGTH_TABLE = 2
WIDTH_TABLE = 2
COORDINATE_LOOSE_X = 1
COORDINATE_LOOSE_Y = 0.3
COORDINATE_RED_BALL_X = 1
COORDINATE_RED_BALL_Y = 0.8
COORDINATE_GREEN_BALL_X = 1.5
COORDINATE_GREEN_BALL_Y = 0.8
RADIUS_LOOSE = 0.2
RADIUS_BALLS = 0.05
MASS_BALLS = 0.3
V0_RED_BALL = 2
    ...
    for j in range(MAX_ALPHA):
        ...
        while round(red_ball.v, 3) > 0 or round(green_ball.v, 3) > 0:
            ...
            if get_distance(red_ball.x, red_ball.y, green_ball.x, green_ball.y) <= 2 * RADIUS_BALLS and\
                    (not is_collided):
                is_collided = True
                normal_x = (green_ball.x - red_ball.x) / get_distance(red_ball.x, red_ball.y, green_ball.x, green_ball.y)
                normal_y = (green_ball.y - red_ball.y) / get_distance(red_ball.x, red_ball.y, green_ball.x, green_ball.y)
                if normal_x == 0:
                ...
                scalar_red_v_transpose = scalar_product(red_ball.vx, red_ball.vy, normal_transpose_x, normal_transpose_y)
                scalar_red_v_normal = scalar_product(red_ball.vx, red_ball.vy, normal_x, normal_y)
                scalar_green_v_transpose = scalar_product(green_ball.vx, green_ball.vy, normal_transpose_x, normal_transpose_y)
                scalar_green_a_normal = scalar_product(green_ball.ax, green_ball.ay, normal_x, normal_y)
                scalar_red_a_transpose = scalar_product(red_ball.ax, red_ball.ay, normal_transpose_x, normal_transpose_y)
                scalar_red_a_normal = scalar_product(red_ball.ax, red_ball.ay, normal_x, normal_y)
                scalar_green_a_transpose = scalar_product(green_ball.ax, green_ball.ay, normal_transpose_x, normal_transpose_y)
